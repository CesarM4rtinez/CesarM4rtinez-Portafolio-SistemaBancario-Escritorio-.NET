...
namespace SisBanca
{
    public partial class Frm_MovimientoAbono : Form
    {
        ...
        private void btn_ticket_click(object sender, EventArgs e)
        {
            ...
            string PaginaHTML_Texto = Properties.Resources.Ticket_Abonos.ToString();
            ...
            if (savefile.ShowDialog() == DialogResult.OK)
            {
                using (FileStream stream = new FileStream(savefile.FileName, FileMode.Create))
                {
                    ...
                    string codigoBarrasBase64 = "123456789"; // Aquí puedes poner el texto que desees codificar

                    BarcodeWriter barcodeWriter = new BarcodeWriter();
                    barcodeWriter.Format = BarcodeFormat.CODE_128;
                    Bitmap bitmap = barcodeWriter.Write(codigoBarrasBase64);
                    ...
                    // Reemplazar la variable en la cadena HTML con la imagen en base64 generada
                    PaginaHTML_Texto = PaginaHTML_Texto.Replace("{{CODIGO_DE_BARRAS}}", base64Image);

                    // Generar el contenido PDF con HTML y el código de barras
                    using (StringReader sr = new StringReader(PaginaHTML_Texto))
                    {
                        XMLWorkerHelper.GetInstance().ParseXHtml(writer, pdfDoc, sr);
                    }
                    ...
                    // Mostrar mensaje de validación
                    MessageBox.Show("El ticket se ha generado con éxito.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
